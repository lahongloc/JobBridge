...
const ApplicationDetail = () => {
	...
	const loadApplication = async () => {
		try {
			...
			console.log("application la: ", res.data.result);
	...
	const handleSendEmail = async (values) => {
		try {
			const res = await APIs.put(
				`${enpoints["updateApplicantStatus"]}/applicationId=${application.id}&isSuitable=${status}`,
				{
			...
			if (res.data.code === 0)
				notification.success({
					...
					description: `Email đã được gửi đến ${application.email}.`,
				});
				...
				title={
					<Tag
						color={
							status === "UNSEEN"
								? "blue"
								: application.status === "Hồ sơ phù hợp"
									? "green"
									: "red"
						}
					>
						...
						<Row>
							<Col span={12}>
								<Typography.Text>
									...
									<a
										href={
											application.curriculumVitae.filePath
										}
										target="_blank"
